...
def get_image(samples, file):
    ...
    path=os.path.join(r"F:\MobileFaceNet-master\aligned_imgs",file.split("/")[0])
    if(not os.path.exists(path)):
        os.mkdir(path)
    save_path = os.path.join(path,file.split("/")[1])
...
def evaluate(model):
    ...
    #     full_path = os.path.join("F:\MobileFaceNet-master",sample['full_path'].replace('\\', '/'))
    #     landmarks=sample['landmarks'][0]
    #     landmarks_str =""
    #     for landmark in landmarks:
    #         landmarks_str+=str(landmark)+" "
    #     info = full_path+" "+landmarks_str
    #     f2.write(info+"\n")
    # f2.close()

    filename = 'data/lfw_test_pair_mini.txt'
